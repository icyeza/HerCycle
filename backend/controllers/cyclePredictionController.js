...
function generateCyclePredictions(startDate, userProfile, cyclesAhead = 3) {
  ...
  const avgCycleLength =
    params.learningData?.cycleLengths?.length > 0
      ? Math.round(
          params.learningData.cycleLengths.reduce((a, b) => a + b, 0) /
            params.learningData.cycleLengths.length
        )
      : params.cycleLength;

  const avgPeriodLength =
    params.learningData?.periodLengths?.length > 0
      ? Math.round(
          params.learningData.periodLengths.reduce((a, b) => a + b, 0) /
            params.learningData.periodLengths.length
        )
      : params.periodLength;

  const avgOvulationDay =
    params.learningData?.ovulationDays?.length > 0
      ? Math.round(
          params.learningData.ovulationDays.reduce((a, b) => a + b, 0) /
            params.learningData.ovulationDays.length
        )
...
function generateSingleCyclePredictions(
  startDate,
  params,
  isCurrentCycle,
  totalCycles
  ...
  const ovulationDay =
    params.ovulationDay || params.cycleLength - params.lutealPhaseLength;
  const ovulationVariance = Math.max(
    1,
    Math.floor(learningFactor * params.ovulationVariance)
  );
  ...
  for (let day = 1; day <= params.cycleLength; day++) {
    ...
    if (day <= params.periodLength) {
      ...
      prediction.flow = getPredictedFlow(
        day,
        params.periodLength,
        params.learningData
      );
...
async function enhancedLearningUpdate(userId, date, dayData) {
  try {
    ...
    if (completeCycles.length >= 2) {
      ...
      const periodLengths = completeCycles
        .map((cycle) => {
      ...
      const ovulationDays = completeCycles
        .map((cycle) => {
          ...
          const fertileDays = cycle.filter(
            (day) => day.type === "fertile" || day.type === "ovulation"
          );
      ...
      completeCycles.forEach((cycle) => {
        ...
        periodDays.forEach((day, index) => {
          ...
          if (!flowPatterns[dayNum][day.flow])
            flowPatterns[dayNum][day.flow] = 0;
        ...
        averages: {
          ...
          ovulationDay:
            ovulationDays.length > 0
              ? Math.round(
                  ovulationDays.reduce((a, b) => a + b, 0) /
  ...
  cycleDays.forEach((day, index) => {
    if (day.type === "period" && currentCycle.length > 0) {
      ...
      if (lastPeriodInCycle) {
        const daysDiff =
          Math.abs(new Date(day.date) - new Date(lastPeriodInCycle.date)) /
          (1000 * 60 * 60 * 24);
        if (daysDiff > 20) {
...
async function regenerateAllPredictions(userId, userProfile) {
  try {
    ...
    console.log(
      `Regenerated ${futurePredictions.length} predictions for user ${userId}`
    );
  } catch (err) {
...
exports.getCycleInsights = async (req, res) => {
  try {
    ...
    const insights = {
      cycleLength:
        learningData.averages?.cycleLength ||
        userProfile?.cycleLength ||
        DEFAULT_CYCLE_PARAMS.cycleLength,
      periodLength:
        learningData.averages?.periodLength ||
        userProfile?.periodLength ||
        DEFAULT_CYCLE_PARAMS.periodLength,
      ovulationDay: learningData.averages?.ovulationDay,
      dataPoints: recentData.length,
      totalCycles: learningData.totalCycles || 0,
      cycleRegularity:
        learningData.cycleRegularity || calculateCycleRegularity(recentData),
      predictionAccuracy: await calculatePredictionAccuracy(req.user.id),
      commonSymptoms: getCommonSymptoms(recentData),
      learningStats: {
        cycleLengthVariance: learningData.cycleLengths
          ? calculateVariance(learningData.cycleLengths)
          : 0,
...
exports.getUpcomingPredictions = async (userId) => {
  const today = new Date().toISOString().split("T")[0];
  ...
  const summary = {
    ...
    fertileWindow: predictions.filter(
      (p) => p.type === "fertile" || p.type === "ovulation"
    ),
  };
